...
def printScreen(board):
   ...
   print(board.idxBlockType)
   for y in range(board.oScreen.get_dy()-CTetris.iScreenDw):
      line = ''
      for x in range(CTetris.iScreenDw, board.oScreen.get_dx()-Tetris.iScreenDw):
         if array[y][x] == 0:
            line += '□'

         elif array[y][x] == 1:
            if board.idxBlockType == 0:
                line = line + TextColor.red + '■' + "\033[0m"
            elif board.idxBlockType == 1:
                line = line + TextColor.green + '■' + "\033[0m"
            elif board.idxBlockType == 2:
                line = line + TextColor.yellow + '■' + "\033[0m"
            elif board.idxBlockType == 3:
                line = line + TextColor.blue + '■' + "\033[0m"
            elif board.idxBlockType == 4:
                line = line + TextColor.purple + '■' + "\033[0m"
            elif board.idxBlockType == 5:
                line = line + TextColor.cyan + '■' + "\033[0m"
            elif board.idxBlockType == 6:
                line = line + TextColor.pink + '■' + "\033[0m"
            else:
                line = line + TextColor.white + '■' + "\033[0m"
         else:
            line += 'XX'
      print(line)
